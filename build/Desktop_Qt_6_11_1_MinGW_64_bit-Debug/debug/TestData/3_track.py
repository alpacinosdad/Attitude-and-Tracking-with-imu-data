...
├─ 获取用于积分的线性加速度
│   └─ accel_raw_linear来自 2. 去重力模块
│           ├─ 世界系方式: [aax, aay, aaz - GRAVITY]
├─ detect_static_by_gyro_and_raw_accel
│   └─ 调用静止点判断库
│
├─ 加速度处理
│   ├─ correct_linear_accel_bias_by_static_segments
│   │   ├─ 认为静止段理论线性加速度应为 0
│   │   ├─ 用静止段均值估计 acceleration residual bias从全程 acceleration 中扣除该 bias
│   └─ 输出 accel_used
│
├─ 加速度积分得到速度
│   └─ integrate_accel_to_velocity_with_static_reset
│       ├─ 对 accel_used 做梯形积分
│       │   └─ velocity += 0.5 * (a_prev + a_curr) * dt
│       └─ 从静止段离开时重新开始积分
│
├─ 速度处理
│   ├─ static=True 时速度置零
│   ├─ remove_velocity_drift_between_static
│   │   ├─ 对中间速度漂移做线性插值估计 从 velocity 中扣除该线性漂移，避免 bias 累积导致速度跳变
│   └─ 输出 velocity_used
│
└─ 速度积分得到位移
    ├─ integrate_velocity_to_displacement_with_static_hold
    │   ├─ 对 velocity_used 做梯形积分
    │   │
    │   └─ static=True 时位置积分量为 0
    ├─ hold_position_during_static
    │   └─ 进一步确保静止段不漂移   静止时 displacement 保持上一帧
    │
    └─ 输出 displacement_used
...
# 使用世界系线性加速度：
#   [aax, aay, aaz - GRAVITY]
USE_AACCEL_AS_WORLD_LINEAR = True
...
# 使用 aax/aay/aaz - g 后，displacement_used 已经是世界系。 所以默认 False。
