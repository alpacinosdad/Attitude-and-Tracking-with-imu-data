...
"""
1.求姿态
├─ mil_handle_init  各种初始化
├─ mil_while_run
│   ├─ Step 1: 四元数
│   │   │   ├─ initSampleCount += 1
│   │   │   └─ if SampleCount >= initSampleTarget（=2前两帧）
│   │   │       ├─ 求平均加速度 avgAx / avgAy / avgAz
│   │   │       ├─ quat_from_accel
│   │   │       │   └─ 用重力方向初始化 roll / pitch，yaw = 0
│   │   │       ├─ quat_to_rotmat
│   │   │       │   └─ 四元数转旋转矩阵=姿态
│   │
│   ├─ Step 2: 静止点判定  调用1.静止点判断库
│   ├─ Step 3: 加速度计姿态修正
│   │   ├─ 如果 EN_ACC_FIX == 1
│   │   │   ├─ acce_fix
│   │   │   │   ├─ 当前四元数估计重力方向
...
│   │   │   ├─ grav_diff = abs(acc_norm - GRAV_FACT)
│   │   │   └─ fix_fact = ACC_FIX_FACT_MAX * exp(-ATTEN_FACT * grav_diff) 用于判断陀螺仪可信度
│   │   └─ 如果 EN_ACC_FIX == 0
│   │       └─ fix_fact = 0
│   ├─ Step 4: gyro 修正值 用于后续积分
│   │   ├─ gx_used = raw_gx - gyroBiasX + egx * fix_fact
│   │   ├─ gy_used = raw_gy - gyroBiasY + egy * fix_fact
│   │   └─ gz_used = raw_gz - gyroBiasZ + egz * fix_fact
│   ├─ Step 5: 四元数积分更新姿态
│   │   ├─ gyro_solu
│   │   │   ├─ 使用 gx_used / gy_used / gz_used 对四元数积分
│   │   │   └─ 四元数归一化
│   │   └─ quat_to_rotmat
│   │       └─ 更新旋转矩阵 rotMat = 姿态
